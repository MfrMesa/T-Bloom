...
import { EmpresaService } from "/src/services/empresa.js";
import { mapBackendToFormEmpresa, mapFormToBackendEmpresa } from "/src/mappers/empresa";
...
export const EmpresaProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            setDatosEmpresa(mapBackendToFormEmpresa(data));
        } catch (e) {
            console.warn("No se pudo cargar perfil de empresa:", e.message);
        }
        };
        load();
    }, [empresaId]);
    ...
    // JSON puro (si necesitas subir logo, haz un endpoint aparte para archivos)
    const guardarPerfil = async (formDataInput) => {
        if (!empresaId) throw new Error("Empresa no definida.");
        const payload = mapFormToBackendEmpresa(formDataInput);
        const actualizado = await EmpresaService.updatePerfil(empresaId, payload);
        setDatosEmpresa(mapBackendToFormEmpresa(actualizado));
        return true;
    };
    ...
    const guardarOfertas = async (ofertas = []) => {
        ...
        ofertas.map(o =>
            EmpresaService.createOferta(empresaId, o)
            .then(() => true)
            .catch(() => false)
        )
        );
    ...
    const obtenerOfertas = async () => {
        if (!empresaId) return [];
        const data = await EmpresaService.getOfertas(empresaId);
    ...
    const obtenerLocales = async () => {
        if (!empresaId) return;
        const data = await EmpresaService.getLocales(empresaId);
